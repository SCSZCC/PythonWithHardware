...
while True:
    cyberpi.display.show_label('按下A键开始计时', 16, 'center', index=0) 
    if cyberpi.controller.is_press('a'):
        cyberpi.display.show_label('计时中', 24, 'center', index=0) 
        for i in range(0, 8):
            for j in range(0, 16, 1):
                cyberpi.led_matrix.on(j, i, 1)
                time.sleep(1)
        cyberpi.led_matrix.clear(1)
        cyberpi.display.show_label('计时结束', '24', 'center', index=0)
