...
def countdown(hour, minute, second):
    while True:
        if hour == 0 and minute == 0 and second == 0 or cyberpi.controller.is_press("b"):
            cyberpi.display.label("倒计时结束", 24, "center")
        ...
        if second == 0:
            ...
            if minute < 0:
                ...
                if hour < 0:
                    hour = 0
        second -= 1
        cyberpi.display.label(str(hour) + ":" + str(minute) + ":" + str(second), 32, "center")
        time.sleep(1)
...
cyberpi.console.clear()
while True:
    cyberpi.display.label(str(h) + ":" + str(m) + ":" + str(s), 16, "center")
    ...
    if cyberpi.controller.is_press("a"):
        cyberpi.display.label(str(h) + ":" + str(m) + ":" + str(s), 24, "center")
        cyberpi.audio.play("beeps")
        begin = True
        countdown(h, m, s)
    time.sleep(0.1)
